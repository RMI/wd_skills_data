...
# Define File Names
raw_backup = 'wd_skills_data/imports/raw_interests_'+ str(date.today()) + '.xlsx'
import_backup = 'wd_skills_data/imports/import_skill_interests_'+ str(date.today()) + '.xlsx'
...
for file in mydir.glob('RMI - Skills Interest *.xlsx'):
...
for file in sourcefiles:
    if file.startswith('RMI - Skills Interest'):
        shutil.move(os.path.join(sourcepath,file), os.path.join(destinationpath,file))

# Update columns
df.rename(columns={'Preferred Name':'name', 'Skill Interests':'skill'}, 
            inplace=True)
...
df_import = pd.melt(df2, id_vars= {'name'}, value_vars= df2[2:len(df2.columns)], value_name='skill', var_name='skill_del')
...
df_import = df_import[df_import['name'].notnull()]
df_import = df_import[df_import['name'] != '']
df_import = df_import[['name', 'skill']]
...
df_import['uid'] = df_import['name'].astype(str)
...
with database_connection.connect() as conn:
    result = conn.execute(text("select worker, interest from worker_skill_interest"))
    df1 = pd.DataFrame(result.fetchall())
    df1.columns = result.keys()

df1['skill'] = df1['interest'].astype(str)
...
df_import = df_import.drop({"index", "uid"}, axis= 1)

df_import.rename(columns={'skill':'interest', 'name':'worker'}, 
            inplace=True)
...
# Import new records
df_import.to_sql(con=database_connection, name='worker_skill_interest', if_exists='append', index=False)
